// Evidence Sources Dashboard - External sources display for active retrieval providers.
...
interface EvidenceSourcesDashboardProps {
...
const EvidenceSourcesDashboard: React.FC<EvidenceSourcesDashboardProps> = ({
  analysis,
  onRefresh,
  onRequestReview,
        ...
        <div className="flex items-start">
          ...
          <div className="flex-1">
            ...
            {onRefresh && (
              <button
                onClick={onRefresh}
                disabled={Boolean(rateLimitInfo.retryAfter && rateLimitInfo.retryAfter > 60)}
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            {onRequestReview && !isEvidenceBacked && !privateBetaMode && (
              <button
        ...
        <>
          {privateBetaMode && (
            <div className="mb-4 rounded-lg border border-slate-700 bg-slate-900/70 px-4 py-3 text-sm text-slate-300">
              Analyst review and publication workflow stay private during the first broad beta. Public users can inspect the evidence bundle without opening internal review actions.
            </div>
          )}
          {/* Retrievals Grid */}
...
const RetrievalCard: React.FC<RetrievalCardProps> = ({ retrieval, index }) => {
  ...
  const getSourceBadge = (source: string) => {
    const badges = {
      exa: { color: 'bg-cyan-900/40 text-cyan-400', label: 'Web' },
      firecrawl: { color: 'bg-indigo-900/40 text-indigo-400', label: 'Crawl' },
      crossref: { color: 'bg-violet-900/40 text-violet-400', label: 'Research' },
      gemini: { color: 'bg-fuchsia-900/40 text-fuchsia-400', label: 'LLM' },
...
export default EvidenceSourcesDashboard;
