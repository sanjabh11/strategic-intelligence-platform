...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const requestedPolicy = typeof body.policyId === 'string' && body.policyId.trim()
      ? normalizePolicyId(body.policyId)
      : null

    const candidatePolicy = requestedPolicy || summary.recommendation.candidatePolicy
    if (!candidatePolicy || !CONSENSUS_POLICIES.includes(candidatePolicy)) {
      return jsonResponse(400, {
        ok: false,
        message: 'No eligible promotion candidate is available.',
        summary,
      })
    }

    if (!force && summary.recommendation.action !== 'promote' && requestedPolicy === null) {
      return jsonResponse(400, {
        ok: false,
        message: 'Promotion thresholds are not met.',
        summary,
      })
    }

    const previousPolicy = normalizePolicyId(state.active_policy)
    const rationale = force
      ? `Manual promotion to ${policyLabel(candidatePolicy)}.`
      : summary.recommendation.reason

    if (!dryRun) {
      await persistReleaseState(admin, stateKey, {
        active_policy: candidatePolicy,
      ...
      await recordReleaseDecision(admin, {
        ...
        next_policy: candidatePolicy,
        recommended_policy: candidatePolicy,
        rationale,
        metrics: summary,
      })
    }
