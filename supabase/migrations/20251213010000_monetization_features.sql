...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'monte_carlo_simulations'
      AND policyname = 'Service role full access to monte_carlo'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      ...
      AND policyname = 'Service role full access to trading_signals'
  ) THEN
...
WHERE tier::text = 'free';

UPDATE tier_limits SET 
  monte_carlo_iterations = 1000,
  has_signal_access = FALSE,
  has_marketplace_access = TRUE
WHERE tier::text IN ('analyst', 'pro', 'academic');

UPDATE tier_limits SET 
  monte_carlo_iterations = 10000,
  has_signal_access = TRUE,
  has_marketplace_access = TRUE
WHERE tier::text = 'enterprise';
