...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'whop_users'
      AND policyname = 'Users can view their own whop data'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'whop_users'
      AND policyname = 'Service role can manage whop data'
  ) THEN
...
-- Analyst Tier: retained for backward compatibility, aligned with Pro
('analyst', 'Analyst', 1900, 19000, 50, 5, 5, 25, 50, TRUE, TRUE, FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, 'email', TRUE, FALSE, FALSE, 1000, FALSE, TRUE),
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'feature_usage'
      AND policyname = 'System can manage usage'
  ) THEN
...
BEGIN
  ...
  SELECT CASE p_feature
    ...
    ELSE -1
  END INTO v_limit
  FROM tier_limits WHERE tier::text = v_tier;
